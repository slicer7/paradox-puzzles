      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card border-border">
        <SheetHeader className="flex-shrink-0">
          ...
          <SheetDescription className="text-muted-foreground">
            {totalItems === 0 ? "Your cart is empty" : `${totalItems} item${totalItems !== 1 ? 's' : ''} awaiting discovery`}
          </SheetDescription>
        </SheetHeader>
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-4 rounded-lg bg-secondary/30 border border-border/50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-display font-semibold text-primary mt-1">{formatPrice(item.price.amount, item.price.currencyCode)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-card">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-display font-bold text-primary">{formatPrice(totalPrice, items[0]?.price.currencyCode || 'USD')}</span>
                </div>
                <Button onClick={handleCheckout} className="w-full bg-primary text-primary-foreground hover:bg-gold-light font-body font-semibold" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
